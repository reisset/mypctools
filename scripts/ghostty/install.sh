...
set_default_terminal() {
    ...
    local backup_file="$HOME/.config/xdg-terminals.list.ghostty-backup"

    echo ""
    read -rp "Set ghostty as default terminal? [y/N]: " set_default
    ...
    if [[ "$desktop_lower" == *"gnome"* ]] && [[ "$desktop_lower" != *"cosmic"* ]]; then
        ...
        if command -v update-alternatives &>/dev/null; then
            ...
            if update-alternatives --list x-terminal-emulator 2>/dev/null | grep -q ghostty; then
                sudo update-alternatives --set x-terminal-emulator "$ghostty_path"
                print_success "Set ghostty as default via update-alternatives"
            else
                sudo update-alternatives --install /usr/bin/x-terminal-emulator x-terminal-emulator "$ghostty_path" 50
                sudo update-alternatives --set x-terminal-emulator "$ghostty_path"
                print_success "Registered and set ghostty as default terminal"
            fi
        else
            print_warning "update-alternatives not found - cannot set default on GNOME"
            print_warning "You may need to set ghostty as default manually in Settings"
        fi
        return 0
    fi
...
EOF
        print_success "Set Super+Enter to launch ghostty"
    ...
    # Create xdg-terminals.list with ghostty first
    if [ -f "$xdg_terminals" ]; then
        grep -v "^com.mitchellh.ghostty.desktop$" "$xdg_terminals" > "$xdg_terminals.tmp" 2>/dev/null || true
        echo "com.mitchellh.ghostty.desktop" > "$xdg_terminals"
    ...
    print_success "Set ghostty as default terminal"
}
