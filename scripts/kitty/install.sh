...
set_default_terminal() {
    ...
    local backup_file="$HOME/.config/xdg-terminals.list.kitty-backup"

    echo ""
    read -rp "Set kitty as default terminal? [y/N]: " set_default
    ...
    if [[ "$desktop_lower" == *"gnome"* ]] && [[ "$desktop_lower" != *"cosmic"* ]]; then
        ...
        if command -v update-alternatives &>/dev/null; then
            ...
            if update-alternatives --list x-terminal-emulator 2>/dev/null | grep -q kitty; then
                ...
                print_success "Set kitty as default via update-alternatives"
            else
                sudo update-alternatives --install /usr/bin/x-terminal-emulator x-terminal-emulator "$kitty_path" 50
                sudo update-alternatives --set x-terminal-emulator "$kitty_path"
                print_success "Registered and set kitty as default terminal"
            fi
        else
            print_warning "update-alternatives not found - cannot set default on GNOME"
            print_warning "You may need to set kitty as default manually in Settings"
        fi
        return 0
    fi
...
EOF
        print_success "Set Super+Enter to launch kitty"
    ...
    # Create xdg-terminals.list with kitty first
    if [ -f "$xdg_terminals" ]; then
        grep -v "^kitty.desktop$" "$xdg_terminals" > "$xdg_terminals.tmp" 2>/dev/null || true
    ...
    print_success "Set kitty as default terminal"
}
